...
- Email/password is transmitted only to Firebase Authentication through the official SDK. Application code does not persist or log the password. At the user's request, SDK-managed authentication sessions persist in IndexedDB/localStorage and are restored on reload. These bearer credentials remain sensitive: protect browser profiles, clear site data/sign out on shared devices, and protect against XSS. A password manager may save the password independently.
