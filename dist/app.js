import {dateKey,score,validate,closeDays,actualTime} from './core.js';
import {createFeedback} from './feedback.js';
import {CloudStore} from './cloud.js';
import {smallPanel,journal} from './task-extras.js';
...
let authInitializing=true,lastDate=dateKey();
...
function sync(manual=false){
  if(authInitializing){if(manual)tell('ログイン状態と共有設定を読み込んでいます…','pending');return;}
  manualSync ||= manual;
...
async function save(message='保存しました'){
  if(authInitializing){tell('ログイン状態の読み込み完了までお待ちください','pending');return false;}
  ...
  closeDays(data);const snapshot=structuredClone(data);
...
const cloud=new CloudStore((user,remote)=>{
  ...
  data=remote;cloudDirty=false;formDirty=false;const closed=closeDays(data);localStorage.setItem(cacheKey(),JSON.stringify(data));render();sync();tell('記録と共有設定を読み込みました');if(closed&&!authInitializing)queueMicrotask(()=>{void save('日付変更時の達成度を記録しました');});
},(remote,revision)=>{
  if(cloudDirty||formDirty){tell('別の端末で更新されました。入力をバックアップしてから「最新データを読み込む」を押してください。','error');return;}
  data=remote;cloud.revision=revision;closeDays(data);localStorage.setItem(cacheKey(),JSON.stringify(data));render();sync();tell('別の端末の記録・通知設定を反映しました');
},error=>tell(`クラウドに接続できません：${error.message}`,'error'));
document.addEventListener('input',e=>{if(e.target.closest('#taskForm,#reviewForm,#settingsForm,#smallTaskForm'))formDirty=true;});
...
bind=function(){
  ...
  if(past){
    ...
    if(!day().reviewed){
      ...
      document.querySelector('#content .layout > div')?.insertAdjacentHTML('beforeend',`<section class="panel"><h2>提出を忘れた日の振り返り</h2><p class="muted">過去日の振り返りを記録できます。朝の当日宣言は必須ですが、翌日の事前宣言は任意です。宣言なしの日は低達成による時間制限の評価対象外です。</p><form id="reviewForm"><label>できたこと・学んだこと<textarea name="learned" required maxlength="2000"></textarea></label><label>原因と、次に変えること<textarea name="next" required maxlength="2000"></textarea></label><div class="actions"><button class="primary">振り返りを確定する</button></div></form></section>`);
      // 過去日の進捗は締め切り後に編集しません。
    }
  }
  originalBind();
...
function accountPanel(){return `<section class="panel"><h2>専用ユーザーと端末間同期</h2><p>${cloud.user?`ログイン中：${esc(cloud.user.email)}<br>${cloud.ready?'クラウド接続済み':'クラウドの読み込みが必要です'}`:cloud.configured?'専用のメールアドレス・パスワードでログインしてください。':'Firebase公開設定をこの端末に登録してください。'}</p><div class="actions">${cloud.user?'<button id="cloudReload">最新データを読み込む</button><button id="migrateGuest">この端末の記録を取り込む</button><button id="logout">ログアウト</button>':`<button id="login" class="primary" ${cloud.configured?'':'disabled'}>専用ユーザーでログイン</button><button id="configureCloud">Firebase接続設定</button>`}</div><p class="muted">指定した一人だけがアクセスできます（Firestore側のルール設定が必要です）。未ログインの記録は自動送信しません。パスワードそのものは保存せず、Firebaseの認証セッションをこの端末に保持します。共有端末ではログアウトしてください。</p></section>`;}
function bindPersistence(){
  if(view==='today'){
    document.querySelector('#content .layout > div')?.insertAdjacentHTML('beforeend',smallPanel(day(),selected===dateKey()));
    if($('#smallTaskForm'))$('#smallTaskForm').onsubmit=async event=>{
      event.preventDefault();if(selected!==dateKey())return tell('当日のタスクだけ追加できます','error');
      const title=new FormData(event.target).get('title').trim();if(!title)return;
      data.days[selected]||={tasks:[]};const tasks=day().smallTasks||=[];if(tasks.length>=100)return tell('小規模タスクは100件までです','error');
      tasks.push({id:crypto.randomUUID(),title,done:false});if(await save('小規模タスクを追加しました'))render();
    };
    document.querySelectorAll('[data-small]').forEach(button=>button.onclick=async()=>{
      if(selected!==dateKey())return tell('日付変更後は進捗を編集できません','error');const task=day().smallTasks[Number(button.dataset.small)];
      if(!task.done){const value=prompt('実際にかかった時間（分）を入力してください',String(task.actualMinutes??0));if(value===null)return;try{task.actualMinutes=actualTime(value);}catch(error){return tell(error.message,'error');}task.completedAt=new Date().toISOString();}else delete task.completedAt;
      task.done=!task.done;if(await save('小規模タスクの進捗を保存しました'))render();
    });
  }
  if(view==='calendar')document.querySelectorAll('[data-day]').forEach(button=>button.onclick=()=>{
    const key=button.dataset.day,d=data.days[key];$('#journal').innerHTML=`<h2>${esc(key)} の記録</h2>${d?`<p>達成度 ${score(d)}% · ${d.endScore===undefined?'当日集計中':'日付変更時に確定'}</p>`:''}${journal(d)}`;
  });
  ...
  document.querySelectorAll('[data-progress]').forEach(b=>b.onchange=async()=>{
    const task=day().tasks[+b.dataset.progress];if(selected!==dateKey()){b.value=task.progress;return tell('日付変更後は進捗を編集できません','error');}
    const progress=Number(b.value);if(progress===100){const value=prompt('実際にかかった時間（分）を入力してください',String(task.actualMinutes??task.minutes));if(value===null){b.value=task.progress;return;}try{task.actualMinutes=actualTime(value);}catch(error){b.value=task.progress;return tell(error.message,'error');}task.completedAt=new Date().toISOString();}else delete task.completedAt;
    task.progress=progress;if(await save('進捗と実績時間を保存しました'))render();
  });
  ...
  if($('#reviewForm'))$('#reviewForm').onsubmit=async e=>{
    ...
    if(!confirm('振り返りを保存しますか？タスクの進捗は日付が変わるまで更新できます。'))return;
  ...
  if($('#carry'))$('#carry').onclick=async()=>{
    ...
    d.tasks.push(...tasks.map(({actualMinutes,completedAt,...task})=>({...task,id:crypto.randomUUID(),progress:0})));data.days[selected]=d;
  ...
  if(authInitializing)document.querySelectorAll('#content input,#content textarea,#content select,#content button').forEach(el=>el.disabled=true);
}
...
render();void cloud.init().then(()=>{authInitializing=false;closeDays(data);render();sync();}).catch(()=>{authInitializing=false;render();tell('Firebase接続を準備できません。接続設定とネットワークを確認してください。','error');});setInterval(()=>sync(),60000);
setInterval(()=>{
  if(authInitializing||saving)return;const current=dateKey();if(current!==lastDate){const changed=closeDays(data);if(changed&&!formDirty)void save('日付変更時の達成度を記録しました');if(!formDirty){if(selected===lastDate)selected=current;render();}lastDate=current;sync();}
},1000);
